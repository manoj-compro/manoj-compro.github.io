...
var SumPlayer = class {
  ...
  getState() {
    const operands = lodash.flatMapDepth(
  ...
  rename(label) {
    this.label = lodash.startCase(lodash.trim(label)) || this.label;
    return this.getState();
  }
  toNumber(value) {
    const normalized = lodash.toFinite(value);
    return Number.isFinite(normalized) ? normalized : 0;
  }
};

// src/components/SumPlayer.ts
var SUM_PLAYER_TAG = `sum-player-v${"2"}`;
